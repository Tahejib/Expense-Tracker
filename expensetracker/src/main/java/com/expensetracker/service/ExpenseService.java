...
public class ExpenseService {
    ...
    public List<Expense> filterExpenses(String username, LocalDate start, LocalDate end, Category category, String keyword) {
        User user = userRepository.findByUsername(username)
                .orElseThrow(() -> new RuntimeException("User not found with username: " + username));
        ...
        // Apply filters conditionally with streams
        return expenses.stream()
                .filter(e -> start == null || !e.getDate().isBefore(start))    // Date >= start
                .filter(e -> end == null || !e.getDate().isAfter(end))         // Date <= end
                .filter(e -> category == null || e.getCategory().equals(category)) // Category match
                .filter(e -> {
                    if (keyword == null || keyword.isEmpty()) return true;
                    String lowerKeyword = keyword.toLowerCase();
                    return e.getTitle().toLowerCase().contains(lowerKeyword);
                })
	...
	public void deleteExpenseForUser(Long id, String username) {
