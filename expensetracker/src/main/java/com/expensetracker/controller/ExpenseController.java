...
public class ExpenseController {
    ...
    // ========================== ADD EXPENSE ============================
	  ...
	  @PostMapping("/expenses/add") public String addExpense(@Valid @ModelAttribute
	  Expense expense, BindingResult result,
	  
	  @AuthenticationPrincipal UserDetails userDetails, RedirectAttributes
	  redirectAttributes) { if (userDetails == null) return "redirect:/login"; if
	  (result.hasErrors()) return "expenses";
	  
	  String username = userDetails.getUsername();
	  expenseService.saveExpenseForUser(expense, username);
	  redirectAttributes.addFlashAttribute("message",
	  "Expense added successfully!"); return "redirect:/expenses"; }
	...
	/*
	 * @PostMapping("/expenses/edit/{id}") public String updateExpense(@PathVariable
	 * Long id,
	 * 
	 * @Valid @ModelAttribute Expense expense, BindingResult result,
	 * 
	 * @AuthenticationPrincipal UserDetails userDetails, RedirectAttributes
	 * redirectAttributes) { if (userDetails == null) return "redirect:/login"; if
	 * (result.hasErrors()) return "add_expense";
	 * 
	 * String username = userDetails.getUsername();
	 * expenseService.updateExpenseForUser(id, expense, username);
	 * redirectAttributes.addFlashAttribute("message",
	 * "Expense updated successfully!"); return "redirect:/expenses"; }
	 */
    // ========================== DELETE EXPENSE ============================
    ...
    public String deleteExpense(@PathVariable Long id, Principal principal) {
        String username = principal.getName();
        expenseService.deleteExpenseForUser(id, username);
        return "redirect:/expenses?message=Expense+deleted+successfully";
    }
    ...
    public String showAddExpenseForm(Model model) {
        model.addAttribute("expense", new Expense());
        return "add_expense"; // must match the filename (without .html)
    }
    ...
    public String saveExpense(
        ...
        if (result.hasErrors()) {
            return "expenses/form"; // ✅ return to the form if validation fails
        }
        ...
        return "redirect:/expenses?message=Expense+added+successfully";
    }
